...
<div class="relative {className}" bind:this={searchContainer}>
  ...
  <div class="relative">
    ...
    <input
      ...
      type="text"
      {placeholder}
      class="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400 transition-colors duration-200"
      autocomplete="off"
      spellcheck="false"
    />
  ...
  {#if showResults && (searchResults.length > 0 || suggestions.length > 0)}
    <div class="absolute z-50 w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg max-h-96 overflow-y-auto">
      
      <!-- Search Results -->
      {#each searchResults as result, index}
        <button
          class="w-full px-4 py-3 text-left hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700 last:border-b-0 transition-colors duration-150 {
        ...
        >
          <div class="flex items-start space-x-3">
            {#if result.post.featured_image_url}
              ...
              />
            {:else}
              <div class="w-12 h-12 bg-gray-200 dark:bg-gray-600 rounded flex items-center justify-center flex-shrink-0">
      ...
      {#if suggestions.length > 0 && (searchResults.length === 0 || searchQuery.length < minQueryLength)}
        {#if searchResults.length > 0}
          <div class="px-4 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
            Suggestions
          </div>
        {/if}
        ...
        {#each suggestions as suggestion, index}
          ...
          <button
            class="w-full px-4 py-2 text-left hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700 last:border-b-0 transition-colors duration-150 {
